        ...
        <section className="py-20 lg:py-30 bg-(--main-color)">
            <div className="Custom-container grid grid-cols-1 md:grid-cols-3 gap-12.5 lg:gap-15.25">
                {Missions.map((mission) => (
                    <div key={mission.id} className="bg-white p-6 rounded-xl flex flex-col justify-between gap-6">
                        <div>
                        ...
                        <div className="flex items-center gap-2 text-(--btn-color) text-ms md:text-base">
                            <p className="">{mission.actionText}</p>
                            <FaArrowRightLong />
                        </div>
                    </div>
                ))}
